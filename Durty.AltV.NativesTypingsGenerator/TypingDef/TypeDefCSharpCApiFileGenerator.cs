...
namespace Durty.AltV.NativesTypingsGenerator.TypingDef
{
    public class TypeDefCSharpCApiFileGenerator
    {
        ...
        private StringBuilder GenerateModule(TypeDefModule typeDefModule)
        {
            ...
            result.Append($"#include \"../../c-api/data/function_table.h\"\n");
            result.Append($"#include \"../../c-api/client.h\"\n");
            ...
            result.Append("inline uint64_t hashes[] = {\n");
            result.Append(string.Join("", typeDefModule.Functions.Select(e => "    " + e.BaseHash + "UL,\n")));
            result.Append("};\n\n");
            
            result.Append("inline void* pointers[] = {\n");
            result.Append(string.Join("", typeDefModule.Functions.Select(e => $"    (void*) Native_{e.Name},\n")));
            result.Append("};\n\n");

            result.Append("void* GetNativeFuncTable() {\n");
            result.Append("    static constexpr function_table_t data {\n");
            result.Append("        std::extent<decltype(hashes)>::value,\n");
            result.Append("        &hashes[0],\n");
            result.Append("        &pointers[0]\n");
            result.Append("    };\n");
            result.Append("    return (void*) &data;\n");
            result.Append("}");
        ...
        private StringBuilder GenerateFunction(TypeDefFunction typeDefFunction)
        {
            ...
            result.Append($"{converter.Convert(null, typeDefFunction.ReturnType.NativeType[0], false)} Native_{typeDefFunction.Name}(bool& success");
